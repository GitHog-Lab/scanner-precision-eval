...
| AI FP filter | 1 model | ✅ shipped | ✅ measured | ✅ **F1 +0.14pp, precision +0.65pp** | Claude Haiku batched 25/req, parallel 8, ~$0.30/scan, `--ai-verify` flag. Optimal threshold = 0.7. Drops 6 stubborn FPs (Solana program IDs, doc placeholders, test fixtures) at 99.57% precision. | Higher thresholds (≥0.9) too conservative — drop mid-confidence TPs. |
| AI FN recovery | 1 model | ✅ shipped | ✅ measured | ⚪ no lift on this eval | High-entropy candidates near credential-context keywords → Haiku batch judgment, ~$0.32/100k lines, `--ai-recover` flag | On the planted-secret eval, the regex engine already catches everything candidate-review could catch. Genuine lift expected on real-world repos with custom/internal credential formats — needs production-repo evaluation. |
